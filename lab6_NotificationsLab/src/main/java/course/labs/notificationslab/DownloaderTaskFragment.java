...
public class DownloaderTaskFragment extends Fragment {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ArrayList<Integer> friends = arguments.getIntegerArrayList("friends");
		Integer[] friendsArray = new Integer[friends.size()];
		friends.toArray(friendsArray);


		// TODO: Start the DownloaderTask
		downloaderTask.execute(friendsArray);
	...
	public class DownloaderTask extends AsyncTask<Integer, Integer, String[]> {
		...
		protected void onPostExecute(String[] data) {
			...
			//Log.d(TAG, "mCallback is null? " + (mCallback == null));

			//This is not null in actual execution
			if(mCallback != null) mCallback.notifyDataRefreshed(data);
		...
		mContext.sendOrderedBroadcast(new Intent(
				...
				new BroadcastReceiver() {
					...
					public void onReceive(Context context, Intent intent) {
						...
						boolean isMainAlive = (result == MainActivity.IS_ALIVE);

						if (!isMainAlive) {
							//debug
							Log.d(TAG, "Main is not alive");

							// TODO: If not, create a PendingIntent using
							// the
							// restartMainActivityIntent and set its flags
							// to FLAG_UPDATE_CURRENT
							PendingIntent aPendingIntent = PendingIntent.getBroadcast(context, 0, restartMainActivityIntent, PendingIntent.FLAG_UPDATE_CURRENT);
